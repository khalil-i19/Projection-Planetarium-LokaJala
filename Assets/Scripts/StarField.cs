...
public class StarField : MonoBehaviour
{
  ...
  public void Start()
  {
    ...
    foreach (StarDataLoader.Star star in stars)
    {
      ...
      if (star.size >= rangeMagnitude.x && star.size <= rangeMagnitude.y)
  ...
  private void FixedUpdate()
  {
    ...
    {
      Camera.main.transform.RotateAround(Camera.main.transform.position, Camera.main.transform.right, Input.GetAxis("Mouse Y") * speed);
      Camera.main.transform.RotateAround(Camera.main.transform.position, Vector3.up, -Input.GetAxis("Mouse X") * speed);
    }
    return;
  }
